...
void Run();
void Connected(const reactor::TcpPtr &connection);
void Received(const reactor::TcpPtr &connection);
void Closed(const reactor::TcpPtr &connection);
...
void Run() {
  reactor::TcpServer tcp_server(1024, "192.168.254.163");
