...
void MyTask::execute() {
  ...
  if (sz > 1)  //中文
  {
    ...
    if (ans) {
      ...
      // 2.对查询词进行分词
      vector<string>
          chars;  //如，查询词：一家人，分词以后chars里分别存放：一 家 人
      parseWord(chars);

      unordered_map<string, set<int>> indexTableCn =
          _pDictCn->getIndexTableCn();
      ...
      for (size_t idx = 0; idx != chars.size(); ++idx) {
        ...
        if (1 == cnt) {
          set<int> sets =
              indexTableCn[chars[idx]];  //返回该字相关联词的词典下标集合
          std::copy(sets.begin(), sets.end(),
                    std::insert_iterator<set<int>>(iset, iset.begin()));
        }
      }
      statistic(iset);
